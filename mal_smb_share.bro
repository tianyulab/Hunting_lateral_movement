...
export {
    ...
    # VirusTotal API key
    const vt_apikey = "这里填写 VT API key" &redef;
...
{
    # If the file "f" for the event has a source type, and if the source type
    # equals SMB, check file hash against VirusTotal
            ...
            if ( |res| > 0)
            {
                if ( res?$body ) 
                {
                    ...
                    if ( |tmp| != 0 )
                    {
                        ...
                        # If the number of positives (number stored in variable
                        # notic) equals or exceeds the threshold, generate a
                        # notice
